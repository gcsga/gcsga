...
import { FakeGame, _defaultMookData, _oldStats } from "./common"
...
describe("Mook generator", () => {
	...
	describe("Skill Parsing", () => {
		...
		it("Skills with points, difficulty, RSL, specialization, name, levels", () => {
			...
			expect(
				_mook.skills.some(
					s => s.name === "Broadsword" && s.level === 16 && s.difficulty === "dx/a" && s.points === 8
				)
			).toBe(true)
			expect(
				_mook.skills.some(
					s => s.name === "Desert Survival" && s.level === 16 && s.difficulty === "per/a" && s.points === 12
				)
			).toBe(true)
			expect(
				_mook.skills.some(
			...
			expect(
				_mook.skills.some(
					s => s.name === "Fast-Talk" && s.level === 15 && s.difficulty === "iq/a" && s.points === 8
				)
			).toBe(true)
			expect(
				_mook.skills.some(
					s => s.name === "Knife" && s.level === 17 && s.difficulty === "dx/e" && s.points === 8
				)
			).toBe(true)
			expect(
				_mook.skills.some(
			...
			expect(
				_mook.skills.some(
					s => s.name === "Merchant" && s.level === 16 && s.difficulty === "iq/a" && s.points === 12
				)
			).toBe(true)
			expect(
				_mook.skills.some(
					s => s.name === "Mountain Survival" && s.level === 15 && s.difficulty === "per/a" && s.points === 8
				)
			).toBe(true)
			expect(
				_mook.skills.some(
					s => s.name === "Two-Handed Sword" && s.level === 17 && s.difficulty === "dx/a" && s.points === 12
				)
			).toBe(true)
		})
	})
	...
	describe("Spell Parsing", () => {
		it("Spells with name-level notation, no points, no specializations, no RSL, no attributes", () => {
			...
			expect(
				_mook.spells.some(
					s => s.name === "Apportation" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Deflect Missile" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Missile Shield" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Poltergeist" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Winged Knife" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
			...
			expect(
				_mook.spells.some(
					s => s.name === "Bravery" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(s => s.name === "Fear" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0)
			).toBe(true)
			expect(
				_mook.spells.some(s => s.name === "Itch" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0)
			).toBe(true)
			expect(
				_mook.spells.some(s => s.name === "Pain" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Panic" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Sense Emotion" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Sense Foes" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Spasm" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Terror" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Create Fire" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Fireball" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Ignite Fire" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Shape Fire" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
			...
			expect(
				_mook.spells.some(s => s.name === "Blur" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Continual Light" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Flash" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Gloom" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(s => s.name === "Hide" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0)
			).toBe(true)
			expect(
				_mook.spells.some(
					s => s.name === "Invisibility" && s.level === 15 && s.difficulty === "iq/h" && s.points === 0
				)
			).toBe(true)
			expect(
				_mook.spells.some(
