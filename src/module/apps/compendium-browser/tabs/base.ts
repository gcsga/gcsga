...
import { ErrorGURPS, htmlQuery } from "@util"
...
export abstract class CompendiumBrowserTab {
	...
	async open(filter?: BrowserFilter): Promise<void> {
		if (filter) {
			if (!this.isInitialized) {
				throw ErrorGURPS(`Tried to pass an initial filter to uninitialized tab "${this.tabName}"`)
			}
	...
	protected filterTraits(
		...
		if (selectedTraits.length || notTraits.length) {
			...
			const fullfilled = selectedTraits.some(t => traits.includes(t))
			// condition === "and"
