...
import { SYSTEM_NAME } from "./data/constants.ts"
...
export class MigrationSummary extends Application<MigrationSummaryOptions> {
	...
	constructor(options: Partial<MigrationSummaryOptions> = {}) {
		...
		this.options.title = options.troubleshoot
			? LocalizeGURPS.translations.gurps.migrations.summary.troubleshoot.title
			: LocalizeGURPS.translations.gurps.migrations.summary.title
	...
	static override get defaultOptions(): ApplicationOptions {
		return {
			...
			width: 400,
			height: "auto",
			template: `systems/${SYSTEM_NAME}/templates/system/migration-summary.hbs`,
		}
	}
	...
	override activateListeners($html: JQuery): void {
		...
		remigrateButton?.addEventListener("click", async () => {
			...
			try {
				...
				await game.gurps.system.remigrate({ from: lowestSchemaVersion })
				this.options.troubleshoot = false
				this.render(false)
			} catch {
				return
