...
import { eFunction, evalFunctionsGCA } from "./function.ts"
import { GCACharacter } from "../import/data-gca.ts"

// Resolver unset.
class EvaluatorGCA {
	resolver: GCACharacter

	operators: Operator[] = evalOperators(true)

	functions: Map<string, eFunction> = evalFunctionsGCA()

	operandStack: Operand[] = []

	operatorStack: expressionOperator[] = []

	constructor(data: { resolver: GCACharacter; operators?: Operator[]; functions?: Map<string, eFunction> }) {
		this.resolver = data.resolver
		this.operators = data.operators ?? evalOperators(true)
		this.functions = data.functions ?? evalFunctionsGCA()
	}
	...
	replaceVariables(expression: string): string {
		const skillRegex =
			/"?SK:(?<name>[A-z -]+)(?<specialization> \([A-z -]+\))?"? (?<sign>[-+]) (?<modifier>[A-z1-9-_:]+)/

		if (expression.match(skillRegex)) {
			const { groups } = skillRegex.exec(expression)!
			console.log(groups!.name)
			console.log(groups!.specialization)
			console.log(groups!.sign)
