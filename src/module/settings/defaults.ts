...
export class SettingsGURPS {

	static get<N extends string, K extends string>(namespace: N, key: K): ClientSettings.Values[`${N}.${K}`] {
		if (game) return game.settings.get(namespace, key)
		return SettingsGURPS.getDefault(namespace, key)
	}

	static getDefault<N extends string, K extends string>(
		namespace: N,
		key: K
	): typeof this.defaultSettings[N][K] {
		return SettingsGURPS.defaultSettings[namespace]?.[key] ?? null
	}
